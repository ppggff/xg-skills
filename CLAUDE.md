...
- **`xg-dev-workflow/`** — a lite, design-centric dev workflow (default since 2026-09-10, card 030): one card = one directory of docs, one working draft (`design.md`, frontmatter `governance: lite`), explicit commitments, one go, continuous execution — `SKILL.md` routes, `references/steps/lite.md` is the procedure, `steps/discuss.md` / `templates/design-lite.md` + `templates/plan.md` / `references/constraints.md` (the closed list of hard rules) / `references/lenses.md` (lens prompt shapes) its companions; lite.md「Triggers」(no size tiers) switches lenses, review, Inv blocks and a split on from the card's content; the lite face is organized by dimension — 流程 `steps/` · 约束 `constraints.md` · 模版 `templates/` · 视角 `lenses.md` + `smell-catalog.md` · 元 `steps/retro.md` — each under a cap (`constraints.md` Cap-1). **存量 cards** (frontmatter `governance` ∈ ledger / doc-gate / doc-native / empty = legacy, read from `requirement.md`) still run the old five-phase flow (需求 → 设计 → 详设 → 实现 → 测试 + 评审), frozen verbatim in `references/legacy/SKILL.md` (bug fixes only) with its steps/templates/refs and frozen forks of the shared files it cites under `references/legacy/`; SKILL.md's entry rule dispatches single-card operations by that field.
...
<skill>/
  ...
  tools/             # Python helpers (no deps beyond stdlib; PyYAML used if present, else a fallback parser)
                     ...
                     #   tools/legacy/ = the 存量-only check half (legacy_checks.py), block_parse.py, check-superseded-phrases.py
                     #   + their tests — frozen with references/legacy/, lazy-loaded only for a non-lite card
```
...
1. **Synced byte-identical copies are declared in `xg-dev-workflow/tools/sync-manifest.txt` and checked by `tools/check-sync.py`.** Each synced file's header also lists its targets (human-readable; the manifest is what the checker parses). Current sets:
   ...
   - After editing one, `cp` it to the others (byte-identical). `register-project.py` and `kb-backlog.py` live only in `xg-knowledge-lite/tools/`; `commit-data-repos.py`, `check-code-refs.py`, `check-sync.py`, `workflow-status.py`, `workflow-checks.py` (the lite half; the 23 存量-only checks are `tools/legacy/legacy_checks.py`, lazy-loaded), `tools/legacy/block_parse.py`, `tools/legacy/check-superseded-phrases.py` (the `--check` implementations — lazy-loaded by workflow-status.py, one-way dependency checks→parsing) and `viewer.py` (+ its `viewer/` assets) live only in `xg-dev-workflow/tools/` (none of these is synced).
...
5. **`log-usage.py` enforces a canonical action vocabulary** (`KNOWN_ACTIONS` in the script). If you add or rename a verb/action in either `SKILL.md`, update that set in `log-usage.py` (and re-sync the copies) — off-vocabulary actions still log but emit a warning and fragment the retro report. The 2026-09-10 flip moved six phase verbs into `legacy/SKILL.md` without deleting them — `KNOWN_ACTIONS` keeps them (存量 cards still log them).
...
6. **Structural edits must backfill their cross-references in the same batch** (the general rule; the anatomy note above and invariant 5 are its file-level and verb-table instances). Concretely:
   - (a) When changing a term, ID scheme, deferral phrase, or mechanism wording that other files cite, grep `SKILL.md` + `references/` of **both** skills for stale references to the old wording and fix them in the same commit batch (**`xg-dev-workflow/references/legacy/**` and `tools/legacy/` are excluded** — their old wording is frozen on purpose; a `SKILL.md「section」` citation inside a legacy step means `legacy/SKILL.md`) — two past evolutions (the 详设 phase introduction, the 010 ledger) skipped this and each left a crop of broken hand-offs (see the 2026-07-27 template-explicitness audit).
