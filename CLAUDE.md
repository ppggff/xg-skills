...
- **`xg-dev-workflow/`** — a design-centric dev workflow. Five phases (需求 requirement → 设计 design → 详设 detail → 实现 plan/implement → 测试 test) plus an M+ close-out **评审** review gate, one requirement = one directory of docs. Two governance modes per card (017): `ledger` (M+ — decision ledger + digest) vs `doc-gate` (XS/S — document-level gates); the `governance:` frontmatter field decides, no field = legacy. A fifth value, `lite` (trial, 2026-09, read from `design.md` when no requirement.md exists), follows `references/steps/lite.md` only — SKILL.md「Lite route」. Verbs: `new`, `requirement`, `design`, `detail`, `plan`, `test`, `investigate`, `diagnose`, `review`, `improve`, `learn`, `change`, `resume`, `park`, `check`, `status`, `retro`.
