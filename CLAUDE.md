...
- **`xg-dev-workflow/`** — a lite, design-centric dev workflow (default since 2026-09-10, card 030): one card = one directory of docs, one working draft (`design.md`, frontmatter `governance: lite`), explicit commitments, one go, continuous execution — `SKILL.md` routes, `references/steps/lite.md` is the procedure, `steps/discuss.md` / `templates/design-lite.md` / `references/constraints.md` (the closed list of hard rules) / `references/lenses.md` (lens prompt shapes) its companions; the lite face is organized by dimension — 流程 `steps/` · 约束 `constraints.md` · 模版 `templates/` · 视角 `lenses.md` + `smell-catalog.md` · 元 `steps/retro.md` — each under a cap (`constraints.md` Cap-1). **存量 cards** (frontmatter `governance` ∈ ledger / doc-gate / doc-native / empty = legacy, read from `requirement.md`) still run the old five-phase flow (需求 → 设计 → 详设 → 实现 → 测试 + 评审), frozen verbatim in `references/legacy/SKILL.md` (bug fixes only) with its steps/templates/refs and frozen forks of the shared files it cites under `references/legacy/`; SKILL.md's entry rule dispatches single-card operations by that field.
