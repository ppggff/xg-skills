...
- **`xg-dev-workflow/`** — a lite, design-centric dev workflow (default since 2026-09-10, card 030): one card = one directory of docs, one working draft (`design.md`, frontmatter `governance: lite`), explicit commitments, one go, continuous execution — `SKILL.md` routes, `references/steps/lite.md` is the procedure, `steps/discuss.md` / `templates/design-lite.md` / `references/constraints.md` its companions. **存量 cards** (frontmatter `governance` ∈ ledger / doc-gate / doc-native / empty = legacy, read from `requirement.md`) still run the old five-phase flow (需求 → 设计 → 详设 → 实现 → 测试 + 评审), frozen verbatim in `references/legacy/SKILL.md` (bug fixes only) with its steps/templates still at their original paths; SKILL.md's entry rule dispatches single-card operations by that field. Verbs: `new`, `investigate`, `diagnose`, `review`, `improve`, `learn`, `resume`, `park`, `check`, `status`, `retro`; the legacy phase verbs (`requirement`, `design`, `detail`, `plan`, `test`, `change`) exist only in `legacy/SKILL.md`.
