...
| park | — (authored inline; orchestrates M3 + `commit-data-repos.py` + grill.md's round-end order + the ledger/facts/KB write disciplines; examined-not-forked: installed `handoff` / `remember` skills — carriers incompatible with the progress.md entry model) | single owner for the leave moment; the write side of M4 session continuity |
| learn | — (authored inline; composes the investigate skeleton — read-only + KB-first + note landing/receipts — with improve's report/chat-gate/roadmap-exit shape and evidence M1) | distill redo-input from existing cards; ten-section report, chat-stop adoption gate, post-go backfill closed list |
