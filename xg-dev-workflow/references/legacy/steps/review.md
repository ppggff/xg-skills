...
4. **Pick the tier, then dispatch.** Tier by stakes — diff size (step 1), requirement sizing,
   ...
   - **standard** (M, ~one-sitting diff) — **three axis agents** (axis shape from the external
     ...
     **Spec axis** (session model capped at opus — SKILL.md「Subagents」) —
     ...
     `references/legacy/simplify-checks.md` (paste them into the axis brief; it assumes no shared memory).
     Where the repo documents no convention for a smell, fall back to the **smell catalog** baseline
     (`references/legacy/smell-catalog.md`); skip anything tooling already enforces. **Invariants axis** (session model, same cap) —
   ...
   **Deep tier — you MUST read `references/legacy/steps/review-deep.md` before running:** it carries the
...
5b. **Saturation verdict (deep tier) — the "another pass?" stop-rule (overlap-dominant vs
   ...
   lives in `references/legacy/steps/review-deep.md`; consult it on any deep run.**
