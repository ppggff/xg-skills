...
2. Write Context / Decision / (optional Alternatives / Consequences). **The `Status:` line is
   a display snapshot derived from the approval authority** (`decisions.md` on ledger cards;
   the referenced design blocks' annotations on doc-native cards — 026's own ADR-0001 is the
   precedent; on a doc-gate card the Status mirrors the owning doc's gate state via its
   Change log line;
