...
yet still diffs as text, and sidesteps the CJK-alignment pain of a hand-drawn ASCII diagram. Fall
back to ASCII only for a trivial diagram or one Mermaid can't express. Either way: label each box
with its responsibility and each arrow with what flows. A diagram that only restates the prose adds
nothing — it must show structure the prose can't.

Mermaid pitfalls (ASCII `;`, subgraph `direction`) and the CJK-width rules for a forced ASCII diagram → see `references/diagram-gotchas.md`.
