#!/usr/bin/env python3
"""legacy_checks.py — the 23 存量-only card checks, split out of workflow-checks.py (031, 2026-09-14).

Frozen with references/legacy/: bug fixes only; deleted with it in phase 3. Loaded lazily by
workflow-checks.py `_legacy()` — never for a lite card. `wc` is the live view of the lite half
(injected before exec; a standalone load, e.g. a test importing this file directly, loads it by
path); the shared helpers are bound from it below so the check bodies read unchanged. Every public
function takes `ws` = the live workflow-status view, as before. check-superseded-phrases.py and
block_parse.py sit beside this file (same-dir lazy loads).
"""
import glob
import os
import re
...
try:
    wc   # injected by workflow-checks.py `_legacy()`
except NameError:
    import importlib.util as _ilu
    _spec = _ilu.spec_from_file_location(
        "workflow_checks", os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "workflow-checks.py"))
    wc = _ilu.module_from_spec(_spec)
    _spec.loader.exec_module(wc)

GATED_DOCS = wc.GATED_DOCS
PHASE_DOC_NAMES = wc.PHASE_DOC_NAMES
_dep_cycles = wc._dep_cycles
_doc_status = wc._doc_status
_gated_docs = wc._gated_docs
_m = wc._m
_strip_code = wc._strip_code
...
)   # the 23 存量-only card checks, in their CARD_ORDER-relative order
ENTRIES_BY_ID = {e[0]: e for e in ENTRIES}
