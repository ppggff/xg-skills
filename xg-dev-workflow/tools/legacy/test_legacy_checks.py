#!/usr/bin/env python3
"""Tests for legacy/legacy_checks.py — the 存量-only card checks (split out of test_workflow_checks.py, 031).
Frozen with the legacy half; bug fixes only."""
import contextlib
...
wc = ws._checks()
lc = wc._legacy()   # the legacy half — patch its globals here, not on wc (the check bodies read their own module)
...
class LongCellHint(unittest.TestCase):
    ...
    def _post_cutoff(self, d):
        ...
        return mock.patch.object(lc, "_file_created",
    ...
    def test_pre_cutoff_grandfathered(self):
        import unittest.mock as mock
        d = self._card("### Req-1 proposed — t\n- 陈述: %s\n" % self.LONG)
        with mock.patch.object(lc, "_file_created", lambda c, r: "2026-08-20"):
            f, s, exs = wc.check_long_cells(d, ws)
        self.assertEqual((f, s), ([], []))
